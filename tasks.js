...
// function closestCalendar(event) {
//   event.stopPropagation();
//   var button = event.target;
//   var closester = button.closest('.micalendar').getBoundingClientRect();
//   console.log('Closester ', closester);
  ...
  while(j < 7) {
    ...
    for(var i = 1; i < 8; i++) {
      ...
      if ((firstDay.dayWeek > i && j == 1) ) { // если меньше чем 1е число текущего месяца - ячейки для предыдущего месяца
        ...
        if (yearToOperate == new Date().getFullYear() && monthToOperate == new Date().getMonth()) {
